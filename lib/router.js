...
export function buildUpstreamRequest(candidate, messages) {
...
async function callProvider(candidate, messages, fetchImpl) {
  if (isMissingApiKey(candidate.provider)) {
    throw new Error(`provider ${candidate.provider.id} is missing apiKey`);
  }

  const request = buildUpstreamRequest(candidate, messages);
...
export function createChatHandler({ providers, routerAuthKey, fetchImpl = fetch }) {
  return async function handler(req, res) {
    ...
    if (req.headers["x-router-key"] !== routerAuthKey) {
    ...
    if (!validateMessages(body.messages)) {
      return sendJson(res, 400, {
        error: "request body must include messages with role and content strings"
      });
    }
    ...
    for (const candidate of candidates) {
      try {
        upstreamResult = await callProvider(candidate, body.messages, fetchImpl);
      } catch (error) {
        attempts.push({
