...
function getRequestAuthToken(req) {
  ...
  const routerKey = req.headers["x-router-key"];
  return typeof routerKey === "string" ? routerKey.trim() : routerKey;
}
...
export function createChatHandler({ providers, providerConfigError, routerAuthKey, fetchImpl = fetch }) {
  return async function handler(req, res) {
    ...
    const configuredRouterAuthKey =
      typeof routerAuthKey === "string" ? routerAuthKey.trim() : routerAuthKey;

    if (!configuredRouterAuthKey) {
      return sendJson(res, 500, { error: "router auth key is not configured" });
    }

    if (getRequestAuthToken(req) !== configuredRouterAuthKey) {
      return sendJson(res, 401, { error: "unauthorized" });
    }

    if (providerConfigError) {
      return sendJson(res, 500, { error: providerConfigError });
    }
